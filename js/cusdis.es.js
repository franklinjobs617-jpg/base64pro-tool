...
const makeIframeContent = (target) => {
  ...
  const cssPath = `/css/cusdis.style.css`;
...
<html>
  <head>
    ...
    <style>
      ...
      /* --- 输入框图标 --- */
      .dark .grid-cols-2 > div { position: relative; }
     ...
     .dark>div>div>textarea[name="reply_content"] { min-height: 240px; resize: vertical; }
      ...
      button.submit-btn:hover { background-color: #1D48ED !important; transform: translateY(-2px); }
       
      /* --- 评论区布局 --- */
      .dark .my-4 {
        padding: 1.5rem 12px !important; border-top: 1px solid #374151 !important;
      }
      .dark .my-4 .flex.items-center, .dark .my-4 .text-gray-500.text-sm {
        display: inline-block !important; vertical-align: baseline;
      }
      .dark .my-4 .flex.items-center { margin-right: 0.75rem; }
      .dark .my-4 .text-gray-500.text-sm { font-size: 0.8rem !important; color: #9CA3AF !important; }
      .dark .my-4 .text-gray-500.my-2 { margin-top: 0.5rem !important; padding-left: 0.5rem !important; }
      .dark .my-4 button[type="button"] {
      ...
      .dark .my-4 button[type="button"]:hover { background-color: #374151 !important; color: #FFFFFF !important; }

      /* --- 优化 1: 回复评论的缩进和引导线样式 --- */
      /* Cusdis 会用一个 div 包裹所有回复 */
      .dark .my-4 > div:not([class]) {
        margin-top: 1rem;
      ...
      /* --- 清理多余元素 --- */
      .dark a[href="https://cusdis.com"], .dark label { display: none !important; }
    </style>
  </head>
  <body>
...
const textarea = document.querySelector('textarea[name="reply_content"]');
    if (textarea && !textarea.dataset.inputListenerAttached) {
      ...
      textarea.dataset.inputListenerAttached = 'true';
          ...
          const replyButtons = document.querySelectorAll('.dark .my-4 button[type="button"]');
